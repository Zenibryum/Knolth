...
public class ContainerCentrifuge extends Container{
    ...
    public ContainerCentrifuge(InventoryPlayer parInventoryPlayer, IInventory parIInventory)
    {
        // DEBUG
        System.out.println("ContainerCentrifuge constructor()");
        
        tileGrinder = parIInventory;
        sizeInventory = tileGrinder.getSizeInventory();
        addSlotToContainer(new Slot(tileGrinder, 
              TileEntityCentrifuge.slotEnum.INPUT_SLOT.ordinal(), 56, 35));
        addSlotToContainer(new SlotGrinderOutput(parInventoryPlayer.player, 
              tileGrinder, TileEntityCentrifuge.slotEnum.OUTPUT_SLOT.ordinal(), 
              116, 35));
    ...
    */
          ...
          int slotIndex)
    {
        ...
        if (slot != null && slot.getHasStack())
        {
            ...
            if (slotIndex == TileEntityCentrifuge.slotEnum.OUTPUT_SLOT
                  .ordinal())
            {
            ...
            else if (slotIndex != TileEntityCentrifuge.slotEnum.INPUT_SLOT
                  .ordinal())
            {
                // check if there is a grinding recipe for the stack
                if (CentrifugeRecipes.instance().getGrindingResult(itemStack2) 
                      != null)
                {
